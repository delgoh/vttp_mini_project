...
public class VisitorRepository {
    ...
    private final String SQL_INSERT_NEW_VISITOR =
        "INSERT INTO visitors VALUEs (?, ?, ?, ?)";
    private final String SQL_GET_VISITOR_BY_VISITOR_ID =
        "SELECT * FROM visitors WHERE visitor_id = ?";
    private final String SQL_GET_VISITOR_BY_EMAIL =
    ...
    private final String SQL_DELETE_VISITOR_BY_VISITOR_ID =
        "DELETE FROM visitors WHERE visitor_id = ?";
    
    public Boolean insertNewVisitor(String newId, String username, String email, String password) {
        return jdbcTemplate.update(SQL_INSERT_NEW_VISITOR, newId, username, email, password) > 0;
    }

    public Optional<Visitor> getVisitorById(String visitorId) {
        List<Visitor> result = jdbcTemplate.query(
            SQL_GET_VISITOR_BY_VISITOR_ID,
            BeanPropertyRowMapper.newInstance(Visitor.class),
            visitorId
        );
    ...
    public Optional<Visitor> getVisitorByEmail(String email) {
        List<Visitor> result = jdbcTemplate.query(
            SQL_GET_VISITOR_BY_EMAIL,
            BeanPropertyRowMapper.newInstance(Visitor.class),email
        );
